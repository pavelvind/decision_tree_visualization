...
def main():
    ...
    st.write("Simple custom decision tree built from the Breast Cancer dataset.")
    st.write(f"Total samples: {len(labels)}, class distribution: {dict(label_count)}")

    tree = get_tree()
    sample_idx = st.slider("Sample index", 0, len(features) - 1, 0)
    sample = features.iloc[sample_idx]
    prediction = classify(sample, tree)

    st.write("Sample features:")
    st.dataframe(sample.to_frame().T)
    st.write(f"Actual label: {labels.iloc[sample_idx]}")
